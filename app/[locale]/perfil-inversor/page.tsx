...
import { Target, KeyRound, Handshake, Lock } from 'lucide-react';
...
const HERO_IMG = '/hero-inversor.jpg';

const BENEFIT_ICONS = [Target, KeyRound, Handshake];

export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  return buildPageMetadata(params.locale, 'perfil-inversor');
}

export default function PerfilInversorPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  const t = createTranslator(locale, 'inversor');
  const seo = getSeo('perfil-inversor', locale);

  const benefits = (t.raw('page.benefits') as Array<{ title: string; desc: string }>) ?? [];

  return (
    <>
      <JsonLd
        data={[
          webPageSchema(locale, 'perfil-inversor'),
          serviceSchema({
            name: seo.title,
            description: seo.description,
            url: pageUrl(locale, 'perfil-inversor'),
            serviceType: 'Investor mandate & buy-side matching (M&A)',
          }),
        ]}
      />
      <HeroSection imageUrl={HERO_IMG} title={t('page.heroTitle')} subtitle={t('page.heroSubtitle')} compact />
      <Breadcrumbs locale={locale} page="perfil-inversor" />
      ...
      <section className="bg-navy py-24">
        <div className="max-w-[1000px] mx-auto px-6">
          <h2 className="text-2xl md:text-3xl font-light text-offwhite tracking-wide text-center">{t('page.benefitsTitle')}</h2>
      ...
      <section className="bg-white py-24">
        <div className="max-w-[700px] mx-auto px-6">
          <div className="text-center">
            <h2 className="text-2xl font-light text-charcoal tracking-wide">{t('page.formTitle')}</h2>
            <p className="mt-3 text-sm text-charcoal/60 font-light">{t('page.formDesc')}</p>
